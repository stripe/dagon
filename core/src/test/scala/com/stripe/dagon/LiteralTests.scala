...
import Literal.{Const, Unary, Binary}

object LiteralTests extends Properties("Literal") {
  case class Box[T](get: T)

  def transitiveClosure[N[_]](l: Literal[N, _], acc: Set[Literal[N, _]] = Set.empty[Literal[N, _]]): Set[Literal[N, _]] = l match {
    case c @ Const(_) => acc + c
    case u @ Unary(prev, _) => if (acc(u)) acc else transitiveClosure(prev, acc + u)
    case b @ Binary(p1, p2, _) => if (acc(b)) acc else transitiveClosure(p2, transitiveClosure(p1, acc + b))
  }
   ...
   */
  def genLiteral: Gen[Literal[Box, Int]] = Gen.frequency((3, genConst),
